...
function Home() {
  ...
  const movies = moviesData.map((data, i) => {
    ...
    return (
      <Movie key={i} updateLikedMovies={updateLikedMovies}  title={data.title} overview={data.overview.slice(0, 250)} poster={data.poster_path} voteAverage={data.vote_average} voteCount={data.vote_count}/>);
      // <div key={i} className={styles.newMoviesContainer}>
      //    <Movie title={movie.title} poster={poster} voteAverage={movie.vote_average} voteCount={movie.vote_count} overview={movie.overview.slice(0, 250)}>;
      // </div>);
      });
